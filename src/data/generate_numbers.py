...
def generate(num_samples: int, seed: int, output_dir: str,
    ...
    (img_root / "val").mkdir(parents=True, exist_ok=True)
    (img_root / "test").mkdir(parents=True, exist_ok=True)
    ...
    for (color, size, digits), split in zip(attributes, split_labels):
        ...
        image_path = f"images/{split}/{image_name}"   # relative path stored in metadata
        ...
        img.save(img_root/split/image_name)
    ...
    with open(meta_dir / "generation_config.json", "w", encoding="utf-8") as f:
