...
(IMAGE_ROOT / "val").mkdir(exist_ok=True, parents=True)
(IMAGE_ROOT / "test").mkdir(exist_ok=True, parents=True)
...
def generate_sample(index, split):
    ...
    image_rel_path = f"images/{split}/{sample_id}.png"
    image_abs_path = IMAGE_ROOT / split / f"{sample_id}.png"
...
def generate_dataset(total_count=1000):
    ...
    for i in range(total_count):
        split = assign_split(i, total_count)
        split_counters[split] += 1
        idx = split_counters[split]
        samples.append(generate_sample(idx, split))
    return samples
