...
def generate_dataset(config: GeneratorConfig) -> None:
    ...
    for sample_index in range(config.num_samples):
        ...
        absolute_image_path = config.output_root / "images" / split / f"{sample_id}.png"
    ...
    for split in ["train", "val", "test"]:
       write_jsonl(config.output_root / "metadata" / f"{split}.jsonl", rows_by_split[split])

    summary = build_summary(all_rows)
    with (config.output_root / "metadata" / "summary.json").open("w", encoding="utf-8") as f:
    ...
    with (config.output_root / "metadata" / "generation_config.json").open("w", encoding="utf-8") as f:
